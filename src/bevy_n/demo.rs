use bevy::prelude::*;
use crossterm::event::{KeyEvent, KeyCode, Event as CrosstermEvent};
...
fn demo_startup(mut commands: Commands) {
    ...
    let mut entity_grid = EntityGrid::new_from_shape("EwALACCAAz7447/vP/7x+AABPh7/+O/7jz/4gAMIAA==").unwrap();

    let mon_entity_1 = commands.spawn((
        Mon(500),
        NodePiece::new("mon"),
    )).id();
...
fn debug_key(mut inputs: EventReader<CrosstermEvent>, nodes: Query<&EntityGrid, With<NDNode>>) {
    for input in inputs.iter() {
        if let CrosstermEvent::Key( KeyEvent { code: KeyCode::Char('d'), ..}) = input {
