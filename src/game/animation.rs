...
impl Animation {
    pub fn next(game: &mut GameState) -> Result<(), String> {
        match game.animation() {
            None => Err("next was called for animation, but there is no animation to be called".to_owned()),
            Some(Animation::EnemyActions(enemy_actions_vec)) => {
                let mut enemy_actions_vec_clone  = enemy_actions_vec.clone();
                let node = game.node_mut().ok_or("Enemy AI animation shouldn't occur when there is no Node".to_owned())?;
                if let Some(enemy_action) = enemy_actions_vec_clone.pop() {
                    match enemy_action {
                        ...
                        _ => {
                            unimplemented!("Enemy action {:?} has not been implemented yet", enemy_action);
                        }
                    }
