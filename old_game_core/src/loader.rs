...
use crate::assets::{Asset, AssetDictionary};
...
pub fn load_asset_dictionary<T: Asset + std::fmt::Debug>(
    config: &Configuration,
) -> Result<AssetDictionary<T>, LoadingError> {
    ...
    Ok(files
        ...
        .reduce(|mut master_dictionary, new_dict| {
...
fn get_all_assets_of_name(
    config: &Configuration,
    extension: &str,
) -> std::io::Result<Vec<(OsString, String)>> {
                            ...
                            if file.file_type().unwrap().is_file() {
                                ...
                                if subextension == Some(extension) {
                                    match read_to_string(file.path()) {
                                        ...
                                        Err(err) => {
                                            log::warn!(
                                                "Unable to read asset file [{:?}], error: [{}]",
                                                path.into_os_string(),
                                                err
                                            );
                                            None
