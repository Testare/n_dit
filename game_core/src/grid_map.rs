...
impl GridMap {
    ...
    pub fn head(&self, item_key: Entity) -> Option<Point> {
        self.entries.get(&item_key).copied()
    }

    /// Returns the back point where the given item is in the grid
    pub fn back(&self, item_key: Entity) -> Option<Point> {
    ...
    pub fn item_at(&self, pt: Point) -> Option<Entity> {
    ...
    pub fn number_map(&self) -> Vec<Vec<usize>> {
        ...
        self.grid
            ...
            .map(|col| {
                col.iter()
                    .map(|sqr_opt| sqr_opt.map(|sqr| sqr.item_key().map(|e|2).unwrap_or(1)).unwrap_or(0))
                    .collect()
            })
            .collect()
    }
    ...
    /// occupied)
    pub fn push_back(&mut self, pt: Point, item_key: Entity) -> bool {
        if self.square_is_free(pt) {
    ...
    pub fn push_front(&mut self, pt: Point, item_key: Entity) -> bool {
    ...
    pub fn put_item(&mut self, pt: Point, item: Entity) -> Option<Entity> {
    ...
    /// Adds many entries to the GridMap. Takes an iterable of tuples of (Entity, Points) where Points
    ...
    pub fn put_entries<P: IntoIterator<Item = Point>, I: IntoIterator<Item = (Entity, P)>>(
        &mut self,
...
impl From<Vec<Vec<bool>>> for GridMap {
...
mod test {
    ...
    fn open_vertical_map<T>(n: Entity) -> GridMap<T> {
        GridMap::from(vec![vec![true; n]; 1])
    }

    fn assert_square_eq<T>(map: &GridMap<T>, pt: Point, item: Option<Entity>, next: Option<Point>) {
    ...
    fn assert_head<T>(map: &GridMap<T>, item_key: Entity, pt: Point) {
