...
use super::{EnteringNode, InNode, Node};
...
fn sys_enter_node_when_ready(
    ...
    nodes: Query<(&Node, Entity, Has<EntityGrid>)>,
) {
    // Note: Node loading kickoff should either happen here or in an op
    for (player_id, node_id) in players_entering.iter() {
        if let Some((_, node_entity, node_is_ready)) =
            nodes.iter().find(|(node, _, _)| node.0 == *node_id)
        {
            if node_is_ready {
